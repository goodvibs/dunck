...
impl Board {
    ...
    pub fn clear_piece_at(&mut self, mask: Bitboard) {
        ...
        for color_int in Color::White as usize..Color::Black as usize + 1 {
            self.color_masks[color_int] &= !mask;
        }
    ...
    pub fn clear_and_get_piece_type_at(&mut self, mask: Bitboard) -> PieceType {
        ...
        for piece_type in PieceType::iter_pieces() {
            ...
            if self.piece_type_masks[piece_type_int] & mask != 0 {
                self.piece_type_masks[piece_type_int] &= !mask;
                return piece_type;
            }
        }
    ...
    pub fn put_piece_type_at(&mut self, piece_type: PieceType, mask: Bitboard) {
        self.piece_type_masks[piece_type as usize] |= mask;
        self.piece_type_masks[PieceType::AllPieceTypes as usize] |= mask;
        self.zobrist_hash ^= unsafe { get_piece_zobrist_hash(Square::from(mask.leading_zeros() as u8), piece_type) };
    }
    ...
    pub fn move_piece_type(&mut self, piece_type: PieceType, dst_mask: Bitboard, src_mask: Bitboard) {
        ...
        unsafe {
            let dst_square = Square::from(dst_mask.leading_zeros() as u8);
            let src_square = Square::from(src_mask.leading_zeros() as u8);
            self.zobrist_hash ^= get_piece_zobrist_hash(dst_square, piece_type);
            self.zobrist_hash ^= get_piece_zobrist_hash(src_square, piece_type);
        }
    }
    ...
    pub fn get_piece_type_at(&self, square_mask: Bitboard) -> PieceType {
        for piece_type in PieceType::iter_pieces() {
            if self.piece_type_masks[piece_type as usize] & square_mask != 0 {
                return piece_type;
            }
        }
        PieceType::NoPieceType
    ...
    pub const fn get_colored_piece_bb(&self, colored_piece: ColoredPiece) -> Bitboard {
        ...
        self.piece_type_masks[piece_type as usize] & self.color_masks[color as usize]
    }
